<template>
  <div class="nav" @click="changeNav($event)">
    ...
    <!-- （声明导航） -->
    <router-link class="nav-list" :to="{path:'/wishwall'}">
      <div class="router-img1 router-img" :class="{'active':(activeNavIndex == 1)}"></div>
      <span>许愿墙</span>
    </router-link>
    <router-link class="nav-list" :to="{path:'/treehole'}">
    ...
    <router-link class="nav-list" :to="{path:'/planet'}">
      <div class="router-img3 router-img" :class="{'active':activeNavIndex == 4}"></div>
      <span>星球</span>
    </router-link>
    <router-link class="nav-list" :to="{path:'/mine'}">
  ...
  methods: {
    ...
    changeNav(e) {
      ...
      if (target.nodeName.toLocaleLowerCase() == "a") {
        ...
        this.activeNavIndex = arr.indexOf(href) + 1;
      } else if (
